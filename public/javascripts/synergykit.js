...
Synergykit.rt = {
    ...
    checkConnection: function(data) {
        if (Synergykit.rt.socket == null) {
            Synergykit.rt.socket = Synergykit.rt.io(data.synergykit.api, {
                secure: true
            })
            Synergykit.rt.socket.on("connect", function() {
                if (data.synergykit.debug) {
                    console.log("Connected " + Synergykit.rt.socket.id)
                }
                for (var i in Synergykit.rt.queueAdd) {
                    Synergykit.rt.addListener(Synergykit.rt.queueAdd[i])
                }
                Synergykit.rt.queueAdd = []
                for (var i in Synergykit.rt.queueRemove) {
                    Synergykit.rt.removeListener(Synergykit.rt.queueRemove[i])
                }
                Synergykit.rt.queueRemove = []
            })
            Synergykit.rt.socket.on("unsubscribed", function(result) {
            ...
            Synergykit.rt.socket.on("disconnect", function() {
                if (data.synergykit.debug) {
                    console.log("Disconnected")
                }
                for (var i in Synergykit.rt.listeners) {
                    Synergykit.rt.socket.removeAllListeners(i)
                }
                var tempListeners = Synergykit.rt.listeners
                Synergykit.rt.listeners = {}
                for (var i in tempListeners) {
                    Synergykit.rt.addListener(tempListeners[i])
                }
            })
        }
    },
    addListener: function(data) {
        ...
        if (data.object) {
            if (data.filter && data.filter.name && data.filter.query) {
                ...
                Synergykit.rt.listeners[name] = data
                if (Synergykit.rt.socket.connected) {
                    Synergykit.rt.socket.emit("subscribe", {
                    ...
                    Synergykit.rt.socket.on(name, function(result) {
                        data.object.set(result)
                        data.callback(data.object)
                    })
                    Synergykit.rt.socket.on("subscribed_" + name, function() {
                        if (data.synergykit.debug) {
                            console.log("Subscribed to collection '" + data.object.collection + "', event '" + data.eventName + "', filter '" + data.filter.name + "' for socket " + Synergykit.rt.socket.id)
                        }
                    })
                } else {
                    Synergykit.rt.queueAdd.push(Synergykit.rt.listeners[name])
                }
            } else {
                ...
                if (Synergykit.rt.socket.connected) {
                    ...
                    Synergykit.rt.socket.on("subscribed_" + name, function() {
                        if (data.synergykit.debug) {
                            console.log("Subscribed to collection '" + data.object.collection + "', event '" + data.eventName + "' for socket " + Synergykit.rt.socket.id)
                        }
                    })
                } else {
                    Synergykit.rt.queueAdd.push(Synergykit.rt.listeners[name])
            ...
            if (Synergykit.rt.socket.connected) {
                ...
                Synergykit.rt.socket.on(data.eventName, function(result) {
                    data.callback(result)
                })
                Synergykit.rt.socket.on("subscribed_" + name, function() {
                    if (data.synergykit.debug) {
                        console.log("Subscribed to event '" + name + "' for socket " + Synergykit.rt.socket.id)
                    }
                })
            } else {
                Synergykit.rt.queueAdd.push(Synergykit.rt.listeners[name])
        ...
        if (data.object) {
            if (data.filter) {
                ...
                if (Synergykit.rt.socket.connected) {
                    ...
                    Synergykit.rt.socket.emit("unsubscribe", {
                        tenant: data.synergykit.tenant,
                        key: data.synergykit.key,
                        token: data.synergykit.token,
                        filter: data.filter,
                    ...
                    Synergykit.rt.socket.on("unsubscribed_" + name, function() {
                        if (data.synergykit.debug) {
                            console.log("Unsubscribed to collection '" + data.object.collection + "', event '" + data.eventName + "', filter '" + data.filter.name + "' for socket " + Synergykit.rt.socket.id)
                        }
                    })
                } else {
                    Synergykit.rt.queueRemove.push(Synergykit.rt.listeners[name])
                }
            } else {
                name = data.eventName + "_" + data.object.collection
                Synergykit.rt.listeners[name] = data
                if (Synergykit.rt.socket.connected) {
                    ...
                    Synergykit.rt.socket.on("unsubscribed_" + name, function() {
                        if (data.synergykit.debug) {
                            console.log("Unsubscribed to collection '" + data.object.collection + "', event '" + data.eventName + "' for socket " + Synergykit.rt.socket.id)
                        }
                    })
                    Synergykit.rt.socket.removeAllListeners(name)
                } else {
                    Synergykit.rt.queueRemove.push(Synergykit.rt.listeners[name])
                }
            }
            ...
            if (Synergykit.rt.socket.connected) {
                ...
                Synergykit.rt.socket.on("unsubscribed_" + name, function() {
                    if (data.synergykit.debug) {
                        console.log("Unsubscribed to event '" + data.eventName + "' for socket " + Synergykit.rt.socket.id)
                    }
                })
                Synergykit.rt.socket.removeAllListeners(name)
            } else {
    ...
    speak: function(data) {
        Synergykit.rt.checkConnection(data)
        Synergykit.rt.socket.emit("speak", {
            tenant: data.synergykit.tenant,
            token: data.synergykit.token,
            eventName: data.eventName,
